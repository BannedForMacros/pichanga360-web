...
export function CanchaCardDashboard({ cancha, onEdit, onDelete }: Props) {
  ...
  const deporteCodigo =
    (cancha.tipoCancha?.deporte?.codigo?.toUpperCase() as DeporteCodigo) ??
    'OTRO'
  const superficieCodigo =
    (cancha.superficie?.codigo?.toUpperCase() as SuperficieCodigo) ??
    'SINTETICO'
  ...
  return (
    <Card className="flex flex-col gap-4 p-0 overflow-hidden">
      <CanchaSVG deporte={deporteCodigo} superficie={superficieCodigo} />
      <div className="flex flex-col gap-3 p-5">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-base font-semibold text-dark">{cancha.nombre}</h3>
            <p className="text-xs text-gray-500">
              {cancha.tipoCancha?.deporte?.nombre ?? '—'} ·{' '}
              {cancha.superficie?.nombre ?? '—'} · {cancha.capacidadJugadores}{' '}
              jugadores
            </p>
          </div>
